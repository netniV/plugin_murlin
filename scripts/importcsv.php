...
if (!$import && !$validate)
{
	print "Usage: php importcsv.php <filepath> [--validate] [--import]\n";
	print "                                    [--verbose] [--lines <linenumbers>]\n\n";
	print "Example:\n";
	print "    php importcsv.php file.csv --validate -l 1,2,32\n";
	print "\n";
	print "Arguments:\n\n";

	print "    --validate          Validate the CSV, if used in conjunction with\n";
	print "     -v                 the --import flag will prompt to continue.\n\n";
	print "    --import            Import the CSV into the database. If used in\n";
	print "     -i                 conjuntion with the --validate flag it will\n";
	print "                        prompt before importing any data.\n\n";

	print "                        NOTE: Regardless of if the --validate flag isn't\n";
	print "                        used, the script will never import data it\n";
	print "                        considers to be in error, ie if it fails to\n";
	print "                        validate a line, that line will be skipped when\n";
	print "                        the import runs\n\n";

	print "    --verbose           Print to screen the contents of the CSV as\n";
	print "                        interpreted by the script. Must be used in\n";
	print "                        conjunction with one of --import or --validate\n\n";

	print "    --lines             Process the line numbers specified. Comma\n";
	print "     -l                 separated.\n\n";

	print "    --force             Will force the import of a URL even if the URL\n";
	print "     -f                 already exists within the mURLin database.\n\n";

	print "    --ignore-duplicates Same as force, site will be imported even if a\n";
	print "                        duplicate exists in the DB.\n\n";
...
function ImportCSV($filepath, $verbose = false, $lines = null, $force = false)
{
	...
	if ($handle) {
		...
		while (!$eof) {
			...
			if (ValidateLine($line, $lineNumber, $force)) {
				...
				$urld   = $line[1];
				$sql    = "SELECT url FROM plugin_mURLin_index WHERE url=?";
				$result = db_fetch_assoc_prepared($sql, array($urld));
				...
				$proxyaddress  = $line[5];
				$proxyusername = $line[6];
				$proxypassword = $line[7];
				...
				if ((count($result) == 0 || $force)) {
					...
					$result = db_execute_prepared($sql, array(
						$host, $url, $text_match, $timeout,
						$proxyserver, $proxyaddress,
						$proxyusername, $proxypassword
					));
	...
	$number_processed = (!is_null($lines)) ? count($lines) : $lineNumber;
...
function VerifyCSV($filepath, &$lineErrors, &$lineNumber, &$lineCount, $verbose = false, $lines = null)
{
	...
	$processedLines = 0; // How many lines have we ACTUALLY dealt with
	$duplicates     = false; // Flag set to true if there are duplicate lines in the file.
	$handle         = fopen("$filepath", "r");
	$lineNumber     = 0;
	$lineCount      = 0;
	...
	if ($handle) {
		...
		while (!$eof) {
			...
			if (ValidateLine($line, $lineNumber)) {
				...
				$result = db_fetch_assoc($sql, array($url));
...
function ValidateLine($line, $lineNumber)
{
	...
	if (is_numeric($line[0])) {
		...
		$sql = "SELECT id FROM host WHERE id = ?";
		$result = db_fetch_cell_prepared($sql, array($line[0]));
